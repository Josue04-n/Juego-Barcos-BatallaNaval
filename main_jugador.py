...
class BatallaNavalApp:
    ...
    def on_data_received(self, raw_data):
        ...
        try:
            ...
            if tipo == "OCULTAR_TABLERO":
                self.ocultar_tablero_colocacion()
        
            elif tipo == "COFRES_POSICIONES":
                posiciones_cofres = data.get("cofres", [])
                for (x, y) in posiciones_cofres:
                    self.tablero.matriz[x][y] = "C"
                self.tablero.coferes = posiciones_cofres
                self.mostrar_mensaje_personalizado(
                    "¡Todo Listo!",
                    "El host ha terminado. ¡Puedes comenzar a jugar!"
                )
                self.quitar_cortina()
                ...
                barco_nombre = data.get("barco_nombre")
                x = data.get("x")
                y = data.get("y")
                if barco_nombre and x is not None and y is not None:
            ...
            print(f"Error inesperado en on_data_received: {e}")
    ...
    def ocultar_tablero_colocacion(self):
        """Muestra una cortina que oculta el tablero de colocación del jugador."""
        if hasattr(self, "canvas_colocacion"):
            try:
                self.img_cortina = ImageTk.PhotoImage(
                    Image.open("images/cortina/cortina.webp").resize((500, 500))
                )
                self.cortina = self.canvas_colocacion.create_image(
                    0, 0, image=self.img_cortina, anchor="nw", tags="cortina"
                )
            except Exception as e:
                print(f"Error al cargar la imagen de la cortina: {e}")
                self.cortina = self.canvas_colocacion.create_rectangle(
                    0, 0, 500, 500, fill="#8B4513", tags="cortina"
                )
        
            self.canvas_colocacion.tag_raise("cortina")
        
            self.canvas_colocacion.itemconfigure('barco', state='hidden')
        
            self.mostrar_mensaje_personalizado(
                "Esperando al host",
                "El host está colocando los barcos. Por favor, espera."
            )




    def quitar_cortina(self):
        """Quita la cortina del tablero de colocación."""
        if hasattr(self, "canvas_colocacion"):
            self.canvas_colocacion.delete("cortina")
    ...
    def _colocar_barco_remoto(self, barco_nombre, x, y):
        ...
        if self.tablero.es_posicion_valida_para_colocar(barco, x, y):
            ...
            for (bx, by) in barco.posicion:
                self.canvas_colocacion.create_image(by * 50, bx * 50, image=self.img_barco, anchor="nw", tags="barco_remoto")
            barco.colocado = True
            barco.boton.config(state=tk.DISABLED)
            self.canvas_colocacion.itemconfigure("barco_remoto", state="hidden")
    ...
    def crear_tablero_colocacion(self):
        ...
        self.canvas_colocacion.create_image(0, 0, image=self.img_fondo_colocacion, anchor="nw")

        for x in range(10):
            for y in range(10):
                self.canvas_colocacion.create_image(y * 50, x * 50, image=self.img_mar, anchor="nw")
                rect = self.canvas_colocacion.create_rectangle(
                    y * 50, x * 50, y * 50 + 50, x * 50 + 50,
                    outline="", fill="", tags=f"celda_{x}_{y}"
                )
    ...
    def iniciar_juego(self, nombre_jugador):
        ...
        if soy_host:
            ...
            lista_cofres = list(self.tablero.coferes)
            data = {
    ...
    def reiniciar_juego(self, ventana):
        if self.peer:
            data = {"tipo": "VOLVER_A_JUGAR", "jugador": self.jugador.nombre}
            self.peer.send_message(json.dumps(data))
        """Reinicia el juego cerrando la ventana actual y creando una nueva instancia."""
        ventana.destroy()
